...
let box_color1 = PovColor.RGB(0.02, 0.02, 1.0)
let box_color2 = PovColor.RGB(1.0, 0.02, 0.02)
...
let () =
  ...
  let texture = Pov.new_texture ~color:box_color1 () in
  let sc = Pov.add_box sc
    ~corner1:(0.0, 0.2, 0.2)
    ~corner2:(0.2, 0.0, 0.0)
    ~translate:( 0.2, 0.0,  1.8)
    ~scale:(_scale 0.6)
    ~texture ()
  in
  ...
  let sc = Pov.add_sphere sc
    ~center:(0.0, 0.0, 0.0)
    ~radius:0.16
    ~translate:( 2.2, 1.6,  1.2)
    ~texture ()
  in
  let sc = Pov.add_sphere sc
    ~center:(0.0, 0.0, 0.0)
    ~radius:0.12
    ~translate:( 2.0, 2.8, -0.6)
    ~texture ()
  in
  let sc = Pov.add_sphere sc
    ~center:(0.0, 0.0, 0.0)
    ~radius:0.10
    ~translate:( 2.8, 1.2, -0.2)
    ~texture ()
  in
  (*
  let sc = Pov.add_sphere sc
    ~center:(0.0, 0.0, 0.0)
    ~radius:0.12
    ~translate:( 1.8, 1.2, -1.2)
    ~texture ()
  in
  *)

  let texture = Pov.new_texture ~color:box_color () in
  let corner1 = (0.0, 0.2, 0.2) in
  let corner2 = (0.2, 0.0, 0.0) in
  let sc =
    let translate1 = ( 1.4, 0.0, -3.6) in
    let translate2 = ( 1.4, 0.4, -3.6) in
    let translate3 = ( 1.4, 0.8, -3.6) in
    let desc = [
      Pov.Box (corner1, corner2, texture, Some translate1, None, None);
      Pov.Box (corner1, corner2, texture, Some translate2, None, None);
      Pov.Box (corner1, corner2, texture, Some translate3, None, None);
    ] in
    PovDesc.add_descs sc desc
  in

...
let box_color2 = PovColor.RGB(0.1, 1.0, 0.1)
...
let () =
  ...
  let p4 = ( 0.0, 0.0,  1.0)  in
  let col1 = PovColor.RGB(0.86, 0.84, 0.82) in
  let col2 = PovColor.RGB(0.66, 0.64, 0.62) in
  let triangles2 = [
    ((p1, p2, p3), col1);
    ((p1, p4, p3), col2);
  ] in

  let translate = Some (0.0, 2.4, 0.0) in
  let rotate, scale = (None, None) in
  let sc =

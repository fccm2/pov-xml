...
let () =
  ...
  (* spheres *)
  let sph_color = PovColor.RGB(0.8, 0.8, 0.76) in
  let c1 = Pov.new_texture ~color:sph_color () in
  let c2 = Pov.new_checker ~color1:PovCol.Blue ~color2:PovCol.Red () in
  let c3 = Pov.new_checker ~color1:PovCol.Cyan ~color2:PovCol.White () in
  let c4 = Pov.new_checker ~color1:PovCol.Blue ~color2:PovCol.Yellow () in
  let sc =
    List.fold_left (fun sc (translate, radius, center, scale, texture) ->
      ...
      ( 0.5, 0.5, -0.9), (1.4), (0.0, 1.4, 0.0), (_scale 0.46), c4;
    ]
  in

...
let () =
  ...
  let finish  = Pov.new_finish ~ambient:0.6 ~diffuse:0.6 ~specular:0.6 () in
  let texture = Pov.new_checker ~color1 ~color2 ~finish () in
  ...
  (* Boxes *)
  let mk_box ~cn ~h ~tr:translation () =
    let corner1 = (-.cn, 0.0, -.cn) in
    let corner2 = (  cn, h,   cn) in
    Pov.Box (corner1, corner2, texture, translation, rotation, scale)
  in

  let rec aux acc n =
    if n < 0 then acc else
    let x = (float (Random.int 40)) /. 10.0 in
    let z = (float (Random.int 20)) /. 10.0 in
    let h = (float (Random.int  6)) /. 10.0 in
    let desc = mk_box ~cn:0.1 ~h:(h) ~tr:(Some(x, 0.0, z)) () in
    aux (desc :: acc) (pred n)
  in
  let descs = aux [] 36 in
  ...
  let sc =
    PovDesc.add_descs sc ( [
      mk_box ~cn:0.4 ~h:0.9 ~tr:(Some(0.0, 0.0,  0.0)) ();
      mk_box ~cn:0.3 ~h:0.8 ~tr:(Some(2.2, 0.0, -0.8)) ();

      mk_box ~cn:0.2 ~h:0.4 ~tr:(Some(1.7, 0.0,  1.2)) ();
      mk_box ~cn:0.2 ~h:0.5 ~tr:(Some(2.0, 0.0,  0.8)) ();
    ] @ descs )
  in

  (* Spheres *)

  let texture = Pov.new_texture ~color:(PovCol.RGB(0.2, 0.7, 0.2)) () in

  let mk_sphere ~cn ~h ~tr:translation () =
    let radius = 0.08 in
    let center = (0.0, 0.04, 0.0) in
    let rotation = None in
    Pov.Sphere (radius, center, texture, translation, rotation, scale)
  in

  let rec aux acc n =
    if n < 0 then acc else
    let x = (float (Random.int 60)) /. 10.0 in
    let z = (float (Random.int 40)) /. 10.0 in
    let h = (float (Random.int  2)) /. 10.0 in
    let desc = mk_sphere ~cn:0.1 ~h:(h) ~tr:(Some(x, 0.0, z)) () in
    aux (desc :: acc) (pred n)
  in
  let descs = aux [] 54 in

  let sc =
    PovDesc.add_descs sc ( descs )
  in

...
let () =
  ...
  let sph_color = PovColor.RGB(0.2, 1.0, 0.2) in
  ...
  (* spheres *)
  let radius = 0.1 in
  let center = (0.0, 0.0, 0.0) in
  let scale = Some (_scale 0.6) in
  let scale2 = Some (_scale 0.4) in
  let translate = Some (-0.8, 0.0, -0.4) in
  let sph_color2 = PovColor.RGB(0.0, 0.1, 1.0) in
  let sph_color3 = PovColor.RGB(1.0, 0.0, 0.1) in
  let sph_color4 = PovColor.RGB(1.0, 1.0, 0.0) in
  let texture = Pov.new_texture ~color:sph_color2 () in
  let rotate = None in
  let radius2 = 0.11 in

  (* spheres-a *)
  let sc = PovDesc.add_descs sc [
    Pov.Sphere (radius, center, texture, translate, rotate, scale)
  ] in

  let sphs =
    List.init 52 (fun _ ->
      let x = rand_num (-2.4, -1.2) in
      let z = rand_num (-1.9, -0.3) in
      let translate = Some (x, 0.0, z) in
      Pov.Sphere (radius, center, texture, translate, rotate, scale)
    )
  in
  let sc = PovDesc.add_descs sc sphs in

  (* spheres-b *)
  let texture2 = Pov.new_texture ~color:sph_color3 () in
  let sphs =
    List.init 36 (fun _ ->
      let x = rand_num (-2.6, -1.2) in
      let z = rand_num (-1.6,  0.2) in
      let translate = Some (x, 0.0, z) in
      Pov.Sphere (radius, center, texture2, translate, rotate, scale)
    )
  in
  let sc = PovDesc.add_descs sc sphs in

  (* spheres-c *)
  let texture3 = Pov.new_texture ~color:sph_color4 () in
  let sphs =
    List.init 52 (fun _ ->
      let x = rand_num (-3.6, -2.1) in
      let z = rand_num (-0.8,  0.8) in
      let translate = Some (x, 0.0, z) in
      Pov.Sphere (radius2, center, texture3, translate, rotate, scale2)
    )
  in
  let sc = PovDesc.add_descs sc sphs in
